...
class ControllersettingSeoUrl extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_setting_seo_url->addSeoUrl($this->request->post);
			...
			$this->response->redirect($this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . $url, true));
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_setting_seo_url->editSeoUrl($this->request->get['url_alias_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $url_alias_id) {
				...
				$this->model_setting_seo_url->deleteSeoUrl($url_alias_id);
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . $url, true)
		...
		$data['add'] = $this->url->link('setting/seo_url/add', 'token=' . $this->session->data['token'] . $url, true);


		$data['delete'] = $this->url->link('setting/seo_url/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$seo_url_total = $this->model_setting_seo_url->getTotalSeoUrls($filter_data);





		$results = $this->model_setting_seo_url->getSeoUrls($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['seo_urls'][] = array(
				...
				'edit'       => $this->url->link('setting/seo_url/edit', 'token=' . $this->session->data['token'] . '&url_alias_id=' . $result['url_alias_id'] . $url, true)
		...
		$data['sort_query'] = $this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . '&sort=query' . $url, true);


		$data['sort_keyword'] = $this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . '&sort=keyword' . $url, true);
		...
		$pagination->url = $this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('setting/seo_url_list', $data));
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['url_alias_id'])) {
			...
			$data['action'] = $this->url->link('setting/seo_url/add', 'token=' . $this->session->data['token'] . $url, true);


		} else {


			$data['action'] = $this->url->link('setting/seo_url/edit', 'token=' . $this->session->data['token'] . '&url_alias_id=' . $this->request->get['url_alias_id'] . $url, true);
		...
		$data['cancel'] = $this->url->link('setting/seo_url', 'token=' . $this->session->data['token'] . $url, true);
		...
		if (isset($this->request->get['url_alias_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$seo_url_info = $this->model_setting_seo_url->getSeoUrl($this->request->get['url_alias_id']);
		...
		$this->response->setOutput($this->load->view('setting/seo_url_form', $data));
	...
	protected function validateForm() {
		...
		if (!$this->user->hasPermission('modify', 'setting/seo_url')) {
		...
		$seo_urls = $this->model_setting_seo_url->getSeoUrlsByKeyword($this->request->post['keyword']);
