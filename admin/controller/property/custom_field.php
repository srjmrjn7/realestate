...
class ControllerPropertyCustomField extends Controller {
	...
	public function index() {
		...
		$this->load->language('property/custom_field');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_property_custom_field->addCustomField($this->request->post);
			...
			$this->response->redirect($this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . $url, true));
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_property_custom_field->editCustomField($this->request->get['custom_field_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach ($this->request->post['selected'] as $custom_field_id) {
				...
				$this->model_property_custom_field->deleteCustomField($custom_field_id);
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . $url, true)
		...
		$data['add'] = $this->url->link('property/custom_field/add', 'token=' . $this->session->data['token'] . $url, true);


		$data['delete'] = $this->url->link('property/custom_field/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		$custom_field_total = $this->model_property_custom_field->getTotalCustomFields();





		$results = $this->model_property_custom_field->getCustomFields($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['custom_fields'][] = array(
				...
				'edit'            => $this->url->link('property/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $result['custom_field_id'] . $url, true)
		...
		$data['sort_name'] = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . '&sort=cfd.name' . $url, true);


		$data['sort_type'] = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.type' . $url, true);


		$data['sort_status'] = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.status' . $url, true);


		$data['sort_sort_order'] = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . '&sort=cf.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('property/custom_field_list', $data));
	...
	protected function getForm() {
		...
		$data['lable_value'] 		= $this->language->get('lable_value');
		$data['lable_validation'] 	= $this->language->get('lable_validation');
		$data['lable_custom_value'] = $this->language->get('lable_custom_value');
		$data['lable_customer_group'] = $this->language->get('lable_customer_group');
		$data['lable_required'] 	= $this->language->get('lable_required');
		...
		if (!isset($this->request->get['custom_field_id'])) {
			...
			$data['action'] = $this->url->link('property/custom_field/add', 'token=' . $this->session->data['token'] . $url, true);


		} else {


			$data['action'] = $this->url->link('property/custom_field/edit', 'token=' . $this->session->data['token'] . '&custom_field_id=' . $this->request->get['custom_field_id'] . $url, true);
		...
		$data['cancel'] = $this->url->link('property/custom_field', 'token=' . $this->session->data['token'] . $url, true);
		...
		if (isset($this->request->get['custom_field_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$custom_field_info = $this->model_property_custom_field->getCustomField($this->request->get['custom_field_id']);
		...
		if (isset($this->request->post['custom_field_description'])) {
			...
			$data['custom_field_description'] = $this->model_property_custom_field->getCustomFieldDescriptions($this->request->get['custom_field_id']);
		...
		if (isset($this->request->post['custom_field_value'])) {
			...
			$custom_field_values = $this->model_property_custom_field->getCustomFieldValueDescriptions($this->request->get['custom_field_id']);
		...
		if (isset($this->request->post['custom_field_customer_group'])) {
			...
			$custom_field_customer_groups = $this->model_property_custom_field->getCustomFieldCustomerGroups($this->request->get['custom_field_id']);
		...
		$this->response->setOutput($this->load->view('property/custom_field_form', $data));
	...
	protected function validateForm() {
		...
		if (!$this->user->hasPermission('modify', 'property/custom_field')) {
