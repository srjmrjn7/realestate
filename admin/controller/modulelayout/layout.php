...
class ControllerModulelayoutLayout extends Controller {
	...
	public function index() {
		...
		$this->load->language('modulelayout/layout');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('modulelayout/layout', 'token=' . $this->session->data['token'] . $url, true));
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('modulelayout/layout', 'token=' . $this->session->data['token'] . $url, true)
		...
		$data['add'] = $this->url->link('modulelayout/layout/add', 'token=' . $this->session->data['token'] . $url, true);


		$data['delete'] = $this->url->link('modulelayout/layout/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['layouts'][] = array(
				...
				'edit'      => $this->url->link('modulelayout/layout/edit', 'token=' . $this->session->data['token'] . '&layout_id=' . $result['layout_id'] . $url, true)
		...
		$data['sort_name'] = $this->url->link('modulelayout/layout', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->link('modulelayout/layout', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('modulelayout/layout_list', $data));
	...
	protected function getForm() {
		...
		$data['text_column_footer3'] = $this->language->get('text_column_footer3');


		$data['text_column_footer4'] = $this->language->get('text_column_footer4');


		$data['text_column_footer5'] = $this->language->get('text_column_footer5');


		$data['text_column_footer'] = $this->language->get('text_column_footer');


		$data['text_column_slider'] = $this->language->get('text_column_slider');


		$data['text_column_testimonial'] = $this->language->get('text_column_testimonial');


		$data['text_content_propty'] = $this->language->get('text_content_propty');
		...
		if (!isset($this->request->get['layout_id'])) {
			...
			$data['action'] = $this->url->link('modulelayout/layout/add', 'token=' . $this->session->data['token'] . $url, true);


		} else {


			$data['action'] = $this->url->link('modulelayout/layout/edit', 'token=' . $this->session->data['token'] . '&layout_id=' . $this->request->get['layout_id'] . $url, true);
		...
		$data['cancel'] = $this->url->link('modulelayout/layout', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('modulelayout/layout_form', $data));
	...
	protected function validateForm() {
		...
		if (!$this->user->hasPermission('modify', 'modulelayout/layout')) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $layout_id) {
			...
			$category_total = $this->model_property_category->getTotalCategoriesByLayoutId($layout_id);
			...
			$information_total = $this->model_property_information->getTotalInformationsByLayoutId($layout_id);
